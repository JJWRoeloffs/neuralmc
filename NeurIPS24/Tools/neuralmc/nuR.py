...
def readForVars(name, module_name, shell_inputs):
    os.system(
        f"../../Tools/ebmc/ebmc_v5_x ../../Benchmarks/{name}.sv --show-symbol-table --bound 0 --top {module_name} > ../../Tools/neuralmc/Model-SMT/{name}_smb_tab.txt"
    )
    data = ""
...
def makeVerilogSampler(
    ...
    inp_out_vars_ordered = order_strings_by_appearance(
        match.group(0), [v for v in inp_out_vars.keys()] + ["clk"]
    )
    ...
    mstr += (
        "\n".join(
            [
                "reg "
                + ("" if value["size"] == 1 else f" [{value['size'] - 1}:0] ")
                + key
                + ";"
...
def trace_file_as_tensor(name, stt_acc, q_max):
    all_t = torch.tensor(
        pd.read_csv(f"../../Tools/neuralmc/Traces/{name}_all_trans.csv")
        .drop_duplicates()
        .values
    )
    spec_states = torch.unique(all_t[:, -1])
    all_states = torch.unique(
        torch.cat((all_t[:, : all_t.size(1) // 2], all_t[:, all_t.size(1) // 2 :])),
        dim=0,
    )
    # tnsr.veiw splits the current and next state, rest of it splits on the automata state
    all_trans_tensors = {
        q.item(): all_t[all_t[:, -1] == q].view(-1, 2, all_t.size(1) // 2)
        for q in spec_states
    }
    for q in range(q_max + 1):
        if q not in spec_states:
            all_trans_tensors[q] = torch.Tensor([])
    all_trans_sizes = {
        value: all_trans_tensors[value].size(0) for value in all_trans_tensors
    }
...
def run_verilator(name):
    os.system(
        f"verilator --binary -j 0 ../../Tools/neuralmc/Verilog-Sampler/{name}_smp.sv"
    )
...
def verilogSMT(name, module_name, state_vars, bits, inp_out_vars, spec_vars):
    os.system(
        f"../../Tools/ebmc/ebmc_v5_x ../../Benchmarks/{name}.sv --smt2 --bound 0 --top {module_name} --outfile ../../Tools/neuralmc/Model-SMT/{name}.smt2"
    )
    smt2_model = ""
    ...
    for key, value in chain(state_vars.items(), inp_out_vars.items()):
        ...
        if value["type"] == "state":
            ...
            smt2_1 = (
                smt2_1.replace("Verilog::ReadWrite.RW@1", "Verilog::ReadWrite.RW@0")
                .replace("Verilog::ReadWrite.emp@1", "Verilog::ReadWrite.emp@0")
                .replace("Verilog::ReadWrite.ful@1", "Verilog::ReadWrite.ful@0")
            )
            smt2_model += "\n" + smt2_1
            inp_out_vars[key]["state_changed"] = False
...
def acc_state_select(stt_acc, q_max, bw_obj, next_vars, rank_before):
    ...
    tm, opt, parser, bvsizeB = bw_obj
    cnds = [
        tm.mk_term(bw.Kind.EQUAL, [next_vars[-1], tm.mk_bv_value(bvsizeB, state)])
        for state in stt_acc
    ]
...
def cond1Check(
    ...
    cnd1_pre = acc_state_select(
        stt_acc, q_max, bw_obj, next_vars, rank_before
    )  # tm.mk_term(bw.Kind.EQUAL, [curr_vars[-1], tm.mk_bv_value(bvsizeB, 1)])
    cnd1_post = tm.mk_term(
        bw.Kind.BV_SLE, [tm.mk_term(bw.Kind.BV_SUB, [rank_before, dt]), rank_after]
    )
    parser.bitwuzla().assert_formula(bAnd([cnd1_pre, cnd1_post], bw_obj))
    with open(
        f"../../Tools/neuralmc/SMT_benchmark/{smt_file_name}_cnd1.smt2", "w"
    ) as file:
        file.write(parser.bitwuzla().print_formula())
    res = parser.bitwuzla().check_sat()
    endV = time.time()
    print(
        f"{colours[pnum % col_num]} [{pnum}] Time VERIFY 1: {endV - beginV}{Style.RESET_ALL}"
    )
    if res == bw.Result.UNSAT:
...
def cond2Check(
    ...
    beginV = time.time()
    parser.bitwuzla().push()
    cnd2_pre = nacc_state_select(
        stt_acc, q_max, bw_obj, next_vars, rank_before
    )  # tm.mk_term(bw.Kind.EQUAL, [curr_vars[-1], tm.mk_bv_value(bvsizeB, 0)])
    cnd2_post = tm.mk_term(bw.Kind.BV_SLT, [rank_before, rank_after])
    parser.bitwuzla().assert_formula(bAnd([cnd2_pre, cnd2_post], bw_obj))
    with open(
        f"../../Tools/neuralmc/SMT_benchmark/{smt_file_name}_cnd2.smt2", "w"
    ) as file:
        file.write(parser.bitwuzla().print_formula())
    res = parser.bitwuzla().check_sat()
    endV = time.time()
    print(
        f"{colours[pnum % col_num]} [{pnum}] Time VERIFY 2: {endV - beginV}{Style.RESET_ALL}"
    )
    if res == bw.Result.UNSAT:
...
def bwPickPiece(bwpnrfs, Svars, bw_obj, q_c, q_max):
    ...
    return tm.mk_term(
        bw.Kind.ITE,
        [
            tm.mk_term(bw.Kind.EQUAL, [Svars[-1], tm.mk_bv_value(bvsizeB, q_c)]),
            bwpnrfs[q_c],
            bwPickPiece(bwpnrfs, Svars, bw_obj, q_c + 1, q_max),
        ],
    )
...
def bwPNRF_encode(pnrf, Svars, F_prec, bw_obj, bits, SMTencode):
    if SMTencode == "new":
        bwpnrfs = {
            q: bwEBV.encode(pnrf[q], Svars, F_prec, bw_obj, bits)[0] for q in pnrf
        }
    elif SMTencode == "old":
        bwpnrfs = {
            q: bwEBV.old_encode(pnrf[q], Svars, F_prec, bw_obj, bits)[0] for q in pnrf
        }
    else:
        breakpoint()
    q_max = max(pnrf.keys())
...
def bwSetListEqual(l1, l2, bw_obj):
    ...
    for i in range(len(l1)):
        res.append(
            tm.mk_term(bw.Kind.EQUAL, [l1[i], tm.mk_bv_value(bvsizeB, int(l2[i]))])
        )
    return res


def TestEncoder(
    pnrf, bw_obj, curr_vars, next_vars, F_prec, bits, all_trans_tensors, SMTencode
):
    tm, opt, parser, bvsizeB = bw_obj
    rank_before = bwPNRF_encode(pnrf, curr_vars, F_prec, bw_obj, bits, SMTencode)
    rank_after = bwPNRF_encode(pnrf, next_vars, F_prec, bw_obj, bits, SMTencode)
    for q, trans in all_trans_tensors.items():
        for bef, aft in zip(trans[:, 0], trans[:, 1]):
            ...
            parser.bitwuzla().assert_formula(
                bAnd(bwSetListEqual(curr_vars, bef, bw_obj), bw_obj)
            )
            parser.bitwuzla().assert_formula(
                bAnd(bwSetListEqual(next_vars, aft, bw_obj), bw_obj)
            )
            res = parser.bitwuzla().check_sat()
            if res == bw.Result.UNSAT:
                print("ERROR!!!")
            ...
            print(
                f"{todecimal(rank_before, bw_obj, bits)/2**F_prec} -> {todecimal(rank_after, bw_obj, bits)/2**F_prec}"
            )

            if not math.isclose(
                todecimal(rank_before, bw_obj, bits) / 2**F_prec,
                pnrf[bef[-1].item()](bef),
                rel_tol=(1000 * 2**-F_prec),
            ):
                breakpoint()
            if not math.isclose(
                todecimal(rank_after, bw_obj, bits) / 2**F_prec,
                pnrf[aft[-1].item()](aft),
                rel_tol=(1000 * 2**-F_prec),
            ):
                breakpoint()
...
def train_an_nrf(
    ...
    print(
        f"{colours[pnum % col_num]}Process {pnum} started with colour {Style.RESET_ALL}"
    )
    TestEncoding = False
    success = False
    seed = pnum
    ...
    if nnP == "Monolithic":
        ...
        pnrf = {
            q: neuralrf.NRF_Clamp(len(scale), scale, clamp_bits, nnP)
            for q in range(q_max + 1)
        }
    optimiserPNRF = {
        q: optim.AdamW(pnrf[q].parameters(), lr=lr, weight_decay=0.01) for q in pnrf
    }
    if TestEncoding:
        TestEncoder(
            pnrf,
    ...
    try:
        while True:
            loss = neuralrf.training(
                pnrf, optimiserPNRF, all_trans_tensors, all_states, stt_acc, pnum, delta
            )
            # breakpoint()
            if loss != 0.0:
            ...
            if cex[0] == None and cex[1] == None:
                print(
                    f"{colours[pnum % col_num]}Yay!!! We have a Ranking function{Style.RESET_ALL}"
                )
                success = True
                break
            if cex[0] != None:
                q_cex = cex[0][1][-1]
                all_trans_tensors[q_cex] = torch.cat(
                    (all_trans_tensors[q_cex], torch.Tensor([cex[0]]))
                )
            if cex[1] != None:
                q_cex = cex[1][1][-1]
                all_trans_tensors[q_cex] = torch.cat(
                    (all_trans_tensors[q_cex], torch.Tensor([cex[1]]))
                )
